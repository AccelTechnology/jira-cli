...
def add_worklog(
    issue_key: str = typer.Argument(..., help="Issue key (e.g., PROJ-123)"),
    time_spent: str = typer.Argument(..., help="Time spent (e.g., '1h 30m', '2d', '4h')"),
    comment: Optional[str] = typer.Option(None, "--comment", "-c", help="Comment for the worklog"),
    started: Optional[str] = typer.Option(None, "--started", "-s", help="Start time (YYYY-MM-DD HH:MM format)"),
    json_output: bool = typer.Option(False, "--json", help="Output raw JSON")
):
...
def update_worklog(
    ...
    time_spent: Optional[str] = typer.Option(None, "--time", "-t", help="Time spent (e.g., '1h 30m', '2d', '4h')"),
    comment: Optional[str] = typer.Option(None, "--comment", "-c", help="Comment for the worklog"),
    started: Optional[str] = typer.Option(None, "--started", "-s", help="Start time (YYYY-MM-DD HH:MM format)"),
    json_output: bool = typer.Option(False, "--json", help="Output raw JSON")
):
    ...
    try:
        ...
        result = client.update_worklog(issue_key, worklog_id, time_spent, comment, started_iso)
