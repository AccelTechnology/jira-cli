...
class ErrorFormatter:
    ...
    def print_formatted_error(
        ...
        message = ErrorFormatter.format_error_message(
            error_type, description, received, expected, examples, suggestions, command_context
        )
    ...
    def validate_choice_parameter(
        ...
        if value not in valid_choices:
            ...
            for choice in valid_choices:
                if choice.lower().startswith(value_lower) or value_lower in choice.lower():
            ...
            ErrorFormatter.print_formatted_error(
                ...
                examples=[f"--{parameter_name.replace('_', '-')} {choice}" for choice in valid_choices[:3]],
                suggestions=suggestions or ["Use one of the valid choices listed above"],
                command_context=command_context
            )
...
def handle_api_error(error: Exception, command_context: str = "") -> None:
    ...
    if "Invalid Jira credentials" in error_message:
        ...
        ErrorFormatter.print_formatted_error(
            ...
            examples=[
                "jira-cli auth test  # Test your connection"
            ],
            command_context="auth test"
        )
